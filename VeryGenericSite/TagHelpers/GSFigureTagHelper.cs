    ...
    public class GSFigureTagHelper : TagHelper, ITagHelperComponent
    {
        ...
        bool Conditionals(string key, object? value)
        {
            while (value is not null)
            {
                ...
                {
                    case "image":
                        Attributes.Add("src", Attributes["image"]);
                        return true;
                    case "src":
                        return true;
                    case "caption":
                    ...
                    case "article-right":
                        return (bool)value;
                    case "source":
                        Attributes.Add("src", Attributes["src"]);
                        return true;
                    default:
        ...
        private TagBuilder BuildTag(ref TagBuilder surrounding, in TagBuilder newTag, TagBuilderDef helper, string attr = "")
        {
            switch (helper)
            {
                ...
                case TagBuilderDef.HasOuterWrapper:
                    ...
                    return surrounding;
                case TagBuilderDef.HasblockWrapper:
                    newTag.AddCssClass(attr);
                    surrounding.InnerHtml.AppendHtml(newTag.RenderStartTag());
                    if (Conditionals("caption", Attributes["caption"]))
                    {
                    ...
                    surrounding.InnerHtml.AppendHtml(newTag.RenderEndTag());
                    if (Conditionals("article", Attributes["article"]))
                    {
                        foreach (var i in Attributes.Keys.Where(x => x.Contains("left") || x.Contains("right")))
                        {
                            if (i.Contains("left"))
                            {
                    ...
                    break;
                case TagBuilderDef.HasInnerWrapper:
                    newTag.AddCssClass(attr);
                    surrounding.InnerHtml.AppendHtml(newTag.RenderStartTag());
                    foreach (var attrib in Attributes.Keys.Where((x) => x.Contains("text")))
                    {
                        if (Conditionals(attrib, Attributes[attrib]))
                        {
                ...
                case TagBuilderDef.HasCaption:
                    if (Conditionals("capupper", Attributes["capupper"]))
                    {
                        ...
                        surrounding.InnerHtml.AppendHtml(newTag.RenderStartTag());
                        surrounding.InnerHtml.Append((string)Attributes["caption"]);
                        surrounding.InnerHtml.AppendHtml(newTag.RenderEndTag());
                    }
